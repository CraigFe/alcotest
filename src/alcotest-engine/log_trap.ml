...
struct
  ...
  let parent_fpath state (path, _) =
    ListLabels.fold_left path ~init:(log_dir state) ~f:(fun acc x ->
        Filename.concat acc (Safe_string.to_string x))

  let output_fpath state ((_, name) as idx) =
    Filename.concat (parent_fpath state idx)
      (Safe_string.to_string name ^ ".output")
  ...
  let get_redirect_or_exn = function
    | Redirect (Suite t) -> t
    | Redirect (Null _) | Inactive -> failwith "internal error: no log location"
    ...
    | Redirect where ->
        let fd =
          ...
          | Suite t ->
              ...
              Platform.open_write_only (output_fpath t idx)
        in
